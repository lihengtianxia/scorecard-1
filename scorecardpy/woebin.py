...
def plot_bin(binx, title, show_iv):
    ...
    '''
    color1,color2=get_options("colors")
    L1,L2=get_options("outcomes")
    ...
    ax2 = ax1.twinx()
    # ax1
    p1 = ax1.bar(ind, binx['good_distr'], width, color=color1)
    p2 = ax1.bar(ind, binx['bad_distr'], width, bottom=binx['good_distr'], color=color2)
    for i in ind:
    ...
    ax2.set_ylabel(L2 + ' probability', color='blue')
    ...
    plt.legend((p2[0], p1[0]), (L2, L1), loc='upper right')
    # show plot 
    # plt.show()
    return fig
